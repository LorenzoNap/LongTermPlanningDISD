...
x <- c("origin_lat", "origin_long", "dest_lat", "dest_long")
colnames(df) <- x
  ...
  for (row in 1:nrow(rotte.agg3)) {
    ...
    lat  <- rotte.agg3[row, "LAT2c"]
        ...
        df[nrow(df) + 1,] = c(-26.134789,28.240528,lat, lon)
...
#Display the data on the globe
    ...
    p <- globejs(img = earth,  lat=latTambo, long=lonTambo, arcs= df, arcsHeight=0.3, arcsLwd=2, arcsColor="#ffff00", arcsOpacity=0.15,
        atmosphere=TRUE)
    p
})
