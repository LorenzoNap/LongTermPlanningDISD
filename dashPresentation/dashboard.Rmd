...
renderPlotly({
  ...
  voli_rotte_clean_sd <- na.omit(voli_rotte_clean_sd[ , c("MTOW","distanza")])
   ...
   km <- kmeans(voli_rotte_clean_sd, centers = numCluster, nstart = 25)
      })
  
  z$doTable <- input$cluster
...
```{r}
    renderTable({
  if(is.null(z$doTable)) return()
      print(z$doTable)
      if( z$doTable >= 4){
          head( xtable(mtcars, digits = 0), n = 6 )
      }
